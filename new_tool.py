...
new_db_pickle_file_path = os.path.join(base_path, 'main_database', 'new_db.pkl')
old_db_pickle_file_path = os.path.join(base_path, 'main_database', 'old_db.pkl')
...
dirs_to_create = [
    ...
    os.path.join(base_path, "main_database"), email_created_path,
...
def database_count():
    try:
        if not os.path.exists(new_db_pickle_file_path):
            return jsonify({"error": "Database file not found"}), 404
        with open(new_db_pickle_file_path, 'rb') as f:
            db = pickle.load(f)
        return jsonify({"count": len(db)})
    ...
    if 'folder' in request.files:
        ...
        for uploaded_file in files:
            if uploaded_file.filename != '' and (uploaded_file.filename.endswith('.csv') or uploaded_file.filename.endswith('.xlsx')):
                file_path = os.path.join(base_path, uploaded_file.filename)
...
def create_email():
    try:
        ...
        return jsonify({"error": f"Failed to load email patterns: {str(e)}"}), 500
    ...
    if 'folder' in request.files:
        ...
        for uploaded_file in files:
            if uploaded_file.filename == '' or not (uploaded_file.filename.lower().endswith(('.csv', '.xlsx'))):
                continue

            # Save the uploaded file to a temporary location (use only basename)
            safe_input_name = os.path.basename(uploaded_file.filename)
            temp_input_path = os.path.join(app.config['UPLOAD_FOLDER'], safe_input_name)
            uploaded_file.save(temp_input_path)

            try:
                df = email_creator_app(temp_input_path, email_patterns)
                ...
                base_name_no_ext = os.path.splitext(safe_input_name)[0]

                email_output = os.path.join(email_created_path, f"{base_name_no_ext}_new_tool_output.csv")
                missing_output = os.path.join(missing_data, f"{base_name_no_ext}_new_tool_missing.csv")

                email_data.to_csv(email_output, index=False)
        ...
        if not combined_email_data.empty:
            combined_email_data.to_csv(os.path.join(email_created_path, "combined_email_output.csv"), index=False)

        log_message = "All files processed — emails generated in created_emails folder!"
    ...
    # ==================== SINGLE FILE UPLOAD ====================
        ...
        try:
            ...
            base_name_no_ext = os.path.splitext(os.path.basename(file.filename))[0]
            ...
            log_message = f"Email generation completed! {len(email_data)} emails created."
...
def automatic_email():
    # Your full long scraping code goes here exactly as before
    # Just make sure at the very end you have:
    log_message = "Automatic email discovery completed!"
    return jsonify({"log": log_message})
