...
def upload_file():
    ...
    if file:
        ...
        try:
            ...
            log_message = f"Error processing file db: {str(e)}"
...
def create_email():
    ...
    if file:
        ...
        try:
            ...
            log_message = f"Error processing file block 2: {str(e)}"
    ...
    if file:
        ...
        if os.path.exists(checkpoint_file):
            ...
            with open(checkpoint_file, "r") as f:
                last_index, last_file, cnt_exit = f.readline().strip().split()
                last_index = int(last_index)
        else:
            last_index = 0
            with open(checkpoint_file, "w") as f:
                f.write(f"{last_index} {output_txt_file} {cnt_exit}\n")
        ...
        file_name = "logs_for_tracking.txt"
        # Construct full path
        log_full_file = os.path.join(folder_name, file_name)
        all_results = []
        ...
        for batch_idx in range(num_batches + (1 if remainder > 0 else 0)):  # Include remainder batch
            ...
            for batch_index in range(num_batches2):
                    ...
                    for query in current_queries:
                        ...
                        final_cnt = int(cnt_exit) + int(g)

                        socketio.emit("log_update", {"logs": f"Total results: {last_index}/{len(unique_companies)} Emails : {final_cnt}"})  # Send logs
                        with open(checkpoint_file, "w") as f:
                            f.write(f"{last_index} {output_txt_file} {final_cnt}\n")
                        random_number = random.randint(0, 2) + random.random()
                        try:
                                res = fetch_google_results(query , num_results=5)
                                time.sleep(3)
                                final_result = []
                                links = [item['link'] for item in res if item.get('link')]
                                q = f"email format for {links[0]}"
                                results = process_run(query , q)
                                ...
                                try:
                                    ...
                                    log_query_result(log_full_file, query, f"-------> {g} --------{final_email}")
                    ...
                    print (f"--------------------------End Batch--------------------------------------", batch_index,"\n")

                    try:
                        update_pickle_file(new_db_pickle_file_path, batch_email)
                        update_pickle_file(old_db_pickle_file_path, batch_email)
                        df = pd.DataFrame(batch_email.items(), columns=['company', 'email pattern'])
                        df.to_csv(file_name+"_file.csv")
                    except:
                        continue
                    
                    print ("sleeping for 50secs")
                    time.sleep(55)
            time.sleep(40)
            print ("sleeping for 45secs")
